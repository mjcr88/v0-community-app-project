...
- **Global Performance Mandate:** "Measure first, optimize second." Every agent must ensure their changes adhere to 2025 performance standards (Core Web Vitals for Web, query optimization for DB, bundle limits for FS). **Critical: All React/Next.js work must consult `@[skills/react-best-practices]`.**
...
### 🔍 Verification Protocol (Code > Docs)

> **MANDATORY**: Documentation (`docs/`, `PRD/`) is often outdated. Always verify logic by reading the `src/` or `app/` code.
> If Code != Docs → **Code wins.**

### 🎨 Nido Design Truth

**All UI/UX work must align with:**
1.  `design/nido_design_system.md` (The source of truth for Tokens & Patterns)
2.  `tailwind.config.ts` (The definition of the theme)
3.  **No generic colors**: Use `bg-muted`, `text-forest-deep`, `border-sand`. NEVER use `bg-gray-200` or `text-green-500`.

### 📱 Mobile First Mandate

Nido is heavily used as a mobile PWA.
1.  **Check the Dock**: Ensure main content has `pb-20` (or `pb-[var(--space-20)]`) so it clears the `MobileDock`.
2.  **Touch Targets**: Minimum 44px for all interactive elements.

### 📝 Documentation & Knowledge Protocol

**MANDATORY: Follow the "Triad" Strategy for information:**
1.  **Product Truth** (`docs/`): All defined specs, manuals, and ADRs. If it's not here, it doesn't exist.
2.  **Business Strategy** (Google Docs): Strategy drafts, financial models, collaborative brainstorming.
3.  **Work Tracking** (Jira): All requirements, tickets, and status.

**Rules:**
- **Implementation Plans**: You MUST ask: "Is documentation needed?" If YES, invoke `documentation-writer`.
- **Jira Check**: Before starting complex work, check Jira. Update the ticket as you work.
- **Decision Records**: Major architectural decisions must be recorded in `docs/05-decisions/`.
