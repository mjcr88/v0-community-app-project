...
2. **Action**:
   ...
   - Write content to `docs-site/docs/developers/` (CRITICAL: Do NOT write to the root `docs/` folder).
...
2. **Action**:
   ...
   - Write content to `docs-site/docs/guides/` (CRITICAL: Do NOT write to the root `docs/` folder).
...
3. **Action**:
   - Create directories: `mkdir -p docs-site/i18n/es/docusaurus-plugin-content-docs/current/`
   - Copy markdown from `docs-site/docs/*` to the aforementioned `es` directory.
   - Translate the markdown files in `es`. 
   - Run `npm run write-translations -- --locale es` in `docs-site/` to extract frontend labels.
