...
/**
 * Toggle requests feature for a tenant
 */
export async function toggleRequestsFeature(tenantId: string, enabled: boolean) {
  const supabase = await createServerClient()

  const { error } = await supabase.from("tenants").update({ requests_enabled: enabled }).eq("id", tenantId)

  if (error) {
    console.error("[v0] Error toggling requests feature:", error)
    throw new Error(error.message)
  }
