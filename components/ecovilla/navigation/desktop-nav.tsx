...
export function DesktopNav({
    ...
    return (
        <TooltipProvider>
            <aside
                className={cn(
                    "hidden md:flex flex-col fixed left-2 top-2 bottom-2 bg-earth-snow border border-earth-pebble/50 rounded-2xl shadow-sm transition-all duration-300 z-30 overflow-hidden",
                    isCollapsed ? "w-16" : "w-64"
                )}
            >
                {/* Header: Tenant Logo & Name */}
                <div className={cn("h-16 flex items-center px-2 border-b border-earth-pebble/50", isCollapsed ? "justify-center" : "justify-start px-4 gap-3")}>
                    <div className="flex-shrink-0 flex items-center justify-center w-8 h-8 rounded-full bg-forest-canopy text-white font-bold text-xs border border-earth-pebble shadow-sm">
                        ESM
                    </div>
                    {!isCollapsed && (
                        <span className="font-bold text-forest-canopy truncate text-sm">
                            {tenantName}
                        </span>
                    )}
                </div>
                                ...
                                {section.items.map((item) => {
                                    ...
                                    return isCollapsed ? (
                                            ...
                                            <TooltipTrigger asChild>
                                                ...
                                                >
                                                    ...
                                                    {item.badge && item.badge > 0 && (
                                                        <span className="absolute top-2 right-2 w-2 h-2 bg-sunrise rounded-full ring-2 ring-white" />
                                                    )}
                                                </Link>
                                            </TooltipTrigger>
                                        ...
                                        >
                                            ...
                                            {item.badge && item.badge > 0 && (
                                                <span className="bg-sunrise text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center">
                                                    {item.badge}
                                                </span>
                                            )}
                                        </Link>
                ...
                {/* Footer Actions */}
                <div className="p-2 border-t border-earth-pebble/50 bg-earth-snow/50 backdrop-blur-sm">
