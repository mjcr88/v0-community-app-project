...
import { flagExchangeListing } from "@/app/actions/exchange-listings"
...
interface FlagListingDialogProps {
  ...
  onFlagSuccess?: () => void // Add callback prop
}

export function FlagListingDialog({
  listingId,
  tenantSlug,
  triggerLabel = "Flag Listing",
  triggerVariant = "outline",
  triggerSize = "default",
  disabled = false,
  initialFlagCount,
  initialHasUserFlagged,
  onFlagSuccess, // Destructure new prop
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await flagExchangeListing(listingId, reason, tenantSlug)

      if (result.success) {
        toast.success("Listing flagged. Admins will review.", {
          duration: 5000,
        })
