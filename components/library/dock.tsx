...
function DockCard({ children, id, className }: DockCardProps) {
  ...
  // Handle click event (simplified to just set active state if needed, but removing bounce)
  const handleClick = () => {
  ...
  let widthSync = useTransform(distance, [-dock.distance, 0, dock.distance], [40, 80, 40])
  ...
  return (
    <div className="flex flex-col items-center gap-1" key={id}>
      ...
      <motion.button
        ...
        className={cn(
          "rounded-lg border aspect-square dark:border-white/5  border-black/5 border-opacity-10 dark:bg-neutral-800 bg-neutral-100 saturate-90 brightness-90 transition-filter duration-200 hover:saturate-100 hover:brightness-112",
          className
        )}
