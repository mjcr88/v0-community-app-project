...
- **Acceptance Criteria**:
  - [ ] Dropdowns in `CreateExchangeListingModal` and `EditExchangeListingModal` only show the valid DB subset.
  - [ ] Attempting to save a listing no longer triggers "check constraint violation" errors.
...
- **Acceptance Criteria**:
  - [ ] Completing a Food pickup does not increase listing quantity.
  - [ ] Completing a Service pickup *does* increase listing quantity (freeing a slot).
  - [ ] Cancelling a transaction *before* pickup still restores inventory for all types.
