-- Migration 047: Extend privacy settings and tenant settings for map features
...
ALTER TABLE tenants
  ADD COLUMN IF NOT EXISTS map_center_coordinates jsonb, -- {"lat": number, "lng": number}
  ADD COLUMN IF NOT EXISTS map_default_zoom integer DEFAULT 15,
  ADD COLUMN IF NOT EXISTS map_boundary_coordinates jsonb; -- Array of {"lat", "lng"} for community boundary
...
COMMENT ON COLUMN tenants.map_center_coordinates IS 'Default center point for community map';
COMMENT ON COLUMN tenants.map_default_zoom IS 'Default zoom level for community map (1-22)';
COMMENT ON COLUMN tenants.map_boundary_coordinates IS 'Polygon boundary of the entire community';
