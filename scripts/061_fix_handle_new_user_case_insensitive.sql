-- Fix the handle_new_user trigger to be case-insensitive when matching emails
-- This ensures that if a user signs up with a different casing than their invite, it still links correctly
...
BEGIN
  ...
  SELECT * INTO existing_user FROM public.users WHERE LOWER(email) = LOWER(new.email);
  
  IF FOUND THEN
    -- Delete the old record (with random UUID from admin creation)
    -- This relies on ON DELETE CASCADE constraints to clean up dependent data, 
    -- or if constraints are missing, orphaned data might remain (but sign-up will succeed).
    ...
    VALUES (
      ...
      existing_user.email, -- Use the email from the existing record (preserving original casing preference if any)
      existing_user.role,
