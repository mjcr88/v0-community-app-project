...
CREATE POLICY "Residents can view events based on visibility"
ON events
FOR SELECT
USING (
  -- Must be in the same tenant as the user
  EXISTS (
    SELECT 1 FROM users 
  ...
  AND (
    ...
    OR
    -- Neighborhood events: user's lot must be in one of the event's neighborhoods
    (
    ...
    (
      ...
      AND (
        ...
        EXISTS (
          ...
          AND u.family_unit_id IS NOT NULL
        )
      )
    )
    OR
...
COMMENT ON POLICY "Residents can view events based on visibility" ON events IS 
'Allows residents to view events based on visibility scope: community (all), neighborhood (user lot in event neighborhoods), private (invited individually or as family), or if they created the event';
